...
- **POIs (restaurants / hotels / groceries / historic sites by boat)** — 38 directory-seeded entries across the lake's main dock towns (restaurants, hotels, groceries) plus 10 curated Historic Landmarks & Sites. Each category has its own sub-toggle in the Layers panel. Tap 📍 → 🍽 POI to add one anywhere on the lake.
...
## App maintenance (origin-scoped)

Two buttons inside the **ⓘ Info modal** cover housekeeping without nuking the rest of your browser:

- **🔄 Check for app updates** — asks the service worker to fetch the latest code from Vercel and reload. Use it if the app feels stuck on an older version. Never touches your trips, buoys, hazards, POIs, or any other local data.
- **🧹 Reset app data…** — wipes *only* this site's localStorage and service-worker cache. Other websites and tabs are unaffected (localStorage and CacheStorage are origin-scoped by the browser — that's a platform guarantee, not an app promise). Confirmation dialog explicitly states the scope. To restore afterwards, use 🛶 → 📥 Import GPX (trips) or re-import the 💾 JSON (all layers).

This replaces the "clear all browser data" workaround that was nuking localStorage. localStorage and CacheStorage are origin-scoped — clearing them for *one* origin doesn't touch any other site, so the in-app reset can be just as surgical as the browser's main "Clear browsing data" menu is broad.
...
- **POIs (restaurants / hotels / groceries):** A 2014 restaurants-by-boat directory + a 2016 hotels-with-docking directory + community contributions. Stale data — verify hours before departing.
- **POIs (Historic Landmarks & Sites):** Tyler reference list 2026-07-12 with one-line primary refs (NH Division of Parks, Lake Winnipesaukee Historical Society, NRHP, New England Historical Society, NH Historical Society). For cultural/lore only — not navigation. The Witches is also seeded as a hazards-layer rock marker (see Crowdsourced hazards below).
...
- [x] **v1.8** — App maintenance: 🔄 update + 🧹 reset buttons (ⓘ Info modal), origin-scoped only — replaces the "clear all browser data" workaround that was nuking localStorage ✅ shipped 2026-09-07
- [ ] **v1.9** — Weather overlay (NOAA station at Wolfeboro)
