...
11. **POIs (restaurants / hotels / groceries / sandbars / landmarks / dive sites / fishing spots)** — 7 per-category layerGroups (`layers.poisRestaurant` / `poisHotel` / `poisGrocery` / `poisSandbar` / `poisLandmark` / `poisDive` / `poisFishing`) + `POI_TYPES` (7 categories: restaurant / hotel / grocery / sandbar / landmark / dive / fishing) + crowdsourced `localStorage.winniPoisV1`. Each POI routes to its category's layerGroup via `poiLayerFor(cat)`. Follows the buoys/hazards CRUD pattern. Seeded on first load from a 2014 restaurants + 2016 hotels reference directory (using nearest `LAUNCHES[]` coords) and Tyler's 2026-07-10 "100 Notable Places" doc; seed is one-shot (flag `winniPoisV1Seeded` keyed to `POI_SEED_VERSION`). User-added POIs carry their own `source` field for verification. v1 seed was 27 entries; v2 added The Dive at Smalls Cove Sandbar (West Alton) with bad coords (~5 nm north of true location); v3 fixed The Dive coords to (43.4700, -71.2600) per Tyler reference list 2026-07-10 item #47; v4 split the single POI toggle into 7 per-category toggles (restaurants, hotels, groceries, sandbars, landmarks, dive sites, fishing spots) and added 45 new entries sourced from the same reference list. Total seeded POIs now 73 (13 restaurants / 15 hotels / 0 groceries / 4 sandbars / 21 landmarks / 10 dive sites / 10 fishing spots). The Dive is still in the restaurant layer (cat: 'restaurant') at v3 coords. Tap 📍 → 🍽 POI to add a new one anywhere on the lake (category dropdown now includes the 4 new types).
...
13. **POIs with non-authoritative coordinates (e.g. floating businesses, seasonal barge bars like The Dive at Smalls Cove Sandbar) are an exception to rule #1, but only via the POI seed path.** Specifically: the entity may go in `POI_SEED` even when coords aren't from an authoritative source, IF (a) `cat` is one of the seven POI categories (`restaurant` / `hotel` / `grocery` / `sandbar` / `landmark` / `dive` / `fishing`) — never `buoy`, `hazard`, or `depth`, (b) the `source` field cites at least the operator's own website or a verifiable reference list (e.g. Tyler's "100 Notable Places" doc), (c) the `note` field explicitly says "coordinates approximate" + a reason (e.g. "anchor moves with the barge" or "verify on chart"), and (d) the entry is bucketed in a section comment like `// ---- West Alton (Smalls Cove Sandbar, ~lat -lng) ----` so the approximate nature is visible in source. Bump `POI_SEED_VERSION` on every addition. Floating businesses and seasonal pop-ups have no fixed location, so don't try to pin them precisely — flag them and move on. If you can verify the actual anchor location later, you can replace the entry (do NOT just edit the seed and re-seed; bump the version so existing users get the fix).

14. **Adding a new POI category is a 5-step checklist, not a one-line edit.** See the comment block above `POI_TYPES` in `index.html` for the canonical version. Quick version: (1) add entry to `POI_TYPES` with `label`, `icon`, `cls`, `layerLabel`; (2) add matching `.poi-marker.<cls>` + `.poi-cat-chip.<cls>` CSS rules; (3) add a `<label class="layer-toggle">` with `id="layerPois<Capitalized>"` in the layer panel; (4) add a `layers.pois<Capitalized> = L.layerGroup()` init in `initMap()` and add it to the `poiToggleBindings` array; (5) bump `POI_SEED_VERSION` if adding seed entries. The `poiLayerFor(cat)` map keeps the routing centralized — update it if you use a different capitalization scheme. Unknown `cat` values fall through to `layers.poisRestaurant` as a safety net so user-added POIs with a typo'd cat still render.
